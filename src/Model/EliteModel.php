<?php
namespace Elite\Plugins\Devtool\Model;

use Elite\Plugins\Devtool\Helper\StringHelper;
...
abstract class EliteModel extends Validation implements TracingInterface
{
    use GetMysql;

    const VERSION_LOCK = 'version';
    /**
     * @var array
     */
    ...
    public function update($selectDb = 'default') {
        ...
        $version = $this->getVersionLock();
        $query = $this->mysql($selectDb)->where($pk, $this->$pk);
        if ($version && $this->_reflectionClass->hasProperty($version)) {
            $reflectionProperty = $this->_reflectionClass->getProperty($version);
            if ($reflectionProperty->isPublic()
                && filter_var($this->$version, FILTER_VALIDATE_INT) !== false) {
                $this->data[$version] = $query->inc(1);
                $query->where($version, $this->$version);
            }
        }
        return $query->update($this->getTableName(), $this->data);
    }
    ...
    public function getVersionLock(): string
    {
        return 'version';
    }
